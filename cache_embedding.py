...
def parse_args(input_args=None):
    ...
    parser.add_argument("--save_cache_embedding",action="store_true",)
...
def main(args):
    os.makedirs(os.path.join(args.cache_embedding_dir, args.category), exist_ok=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():                
                ...
                if args.save_cache_embedding:
                    for i, (im_name, c_name) in enumerate(zip(batch["im_name"], batch["c_name"])):
                        cache_embedding_dict = dict(
                            latents=latents.cpu()[i],
                            masked_latents=masked_latents.cpu()[i],
                            pose_latents=pose_latents.cpu()[i],
                            cloth_latents=cloth_latents.cpu()[i],
                            image_embeds_cloth=image_embeds_cloth.cpu()[i],

                            prompt_embeds=prompt_embeds.cpu()[i],
                            prompt_embeds_cloth=prompt_embeds_cloth.cpu()[i],
                            text_embeds=added_cond_kwargs["text_embeds"].cpu()[i],
                            time_ids=added_cond_kwargs["time_ids"].cpu()[i],
                        )
                        torch.save(cache_embedding_dict, os.path.join(args.cache_embedding_dir, args.category, f"im_{im_name}_c_{c_name}.pt"))
            ...
            num_sample += bsz
            logs = {"time": time.time()-s, "num_sample": num_sample}
