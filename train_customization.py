    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        #     # logger.info(f"data iterated: {time.time() - s}")
            with accelerator.accumulate(unet):
                ...
                for i, (human_image_path, cloth_image_path) in enumerate(zip(batch["im_name"], batch["c_name"])):
                    human_img = Image.open(human_image_path).resize((args.width, args.height))
                    cloth_img = Image.open(cloth_image_path).resize((args.width, args.height))
                    ...
                    inpaint_mask, mask_gray = get_mask_location('dc', category_dict_utils[args.category], model_parse, keypoints)
                    
                    image.append(transform_to_tensor(human_img))
                    pose_image.append(transform_to_tensor(pose_img))
                    cloth_image.append(transform_to_tensor(cloth_img))
                    cloth_clip_image.append(clip_processor(images=cloth_img, return_tensors="pt").pixel_values[0])
                    mask.append(mask_processor(inpaint_mask))

                    caption = "model is wearing a " + caption
                    caption_cloth = "a photo of " + caption_cloth

                    prompt_model.append(caption)
                    prompt_cloth.append(caption_cloth)

                image = torch.stack(image).to(accelerator.device, non_blocking=True) # [-1, 1]
                pose_image = torch.stack(pose_image).to(accelerator.device, non_blocking=True) # densepose # [-1, 1]
                cloth_image = torch.stack(cloth_image).to(accelerator.device, non_blocking=True) # cloth image
                cloth_clip_image = torch.stack(cloth_clip_image).to(accelerator.device, non_blocking=True) # cloth image after clip processor

                mask = torch.stack(mask).to(accelerator.device, non_blocking=True) # mask
