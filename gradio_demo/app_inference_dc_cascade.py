...
def color_to_gray(color_image, color_map):
    gray_image = np.zeros((color_image.shape[0], color_image.shape[1]), dtype=np.uint8)
...
def main():
    ...
    # output_dir = "results/cascade_inference/idm-vton-dc/dresscode_upper_human"
    output_dir = "results/cascade_inference/idm-vton-dc/dresscode_dress_human-debug"
    ...
    # output_dir = "results/idm-vton-dc-lower-body-lr-1e-5-bsz-32-bf16-add_noise_t_to_densepose_latent/checkpoint-3000/debug"
    ...
    unet = UNet2DConditionModel.from_pretrained(
        ...
        # "/root/idm-vton-dc-lower-body-lr-1e-5-bsz-32-bf16-add_noise_t_to_densepose_latent/checkpoint-3000",
        subfolder="unet",
    ...
    def start_tryon(human_img,garm_img,garment_des,is_checked,is_checked_crop,denoise_steps,seed, category, neg_garment_des=""):
        ...
        if is_checked:
            ...
            # 获取cv2.COLORMAP_PARULA颜色映射
            color_map = cv2.applyColorMap(np.arange(0, 256, dtype=np.uint8).reshape(1, -1), cv2.COLORMAP_PARULA).reshape(-1, 3)

            # 将彩色图像转换回灰度图像
            pose_img_matrix = color_to_gray(pose_img, color_map)
            # np.savetxt('array.txt', pose_img_matrix, fmt='%d', delimiter=',')
            ...
            mask, mask_gray = get_mask_location('dc', category_dict_utils[category], model_parse, keypoints, pose_img_matrix)
        ...
        # Image.fromarray(np.concatenate([np.array(human_img), np.array(pose_img_pil), np.array(model_parse.resize((768,1024)).convert("RGB")), np.array(mask.convert("RGB"))], 1)).save("human_mask.png")
        # exit()
                ...
                with torch.no_grad():
                    ...
                    with torch.inference_mode():
                        ...
                        images = pipe(
                            ...
                            guidance_scale=2.0,
                            pose_noise_aug = 0.02 if "add_noise_t_to_densepose_latent" in output_dir else 0.0,
                        )[0]
    ...
    use_dresscode = True
    use_test_ali_human = False
    use_upper_human = False
    use_dress_human = True
    ...
    if use_dresscode:
        ...
        if use_test_ali_human:
            human_list = sorted(os.listdir("/apdcephfs_cq8/share_1367250/yijicheng/data/ft_local/test_ali"))
        else:
            human_list = []

        dresses_test_pairs_unpaired = os.path.join("/apdcephfs_cq8/share_1367250/yijicheng/data/ft_local/DressCode" , "dresses", f"test_pairs_unpaired.txt")
        with open(dresses_test_pairs_unpaired, "r") as f:
        ...
        upper_garment_list = upper_garment_list[:len(human_list)][:10]
        lower_garment_list = lower_garment_list[:len(human_list)][:10]
        human_list = human_list[:10]
            ...
            if use_test_ali_human:
                ...
                examples.append(
                    (
                        ...
                        os.path.join("/apdcephfs_cq8/share_1367250/yijicheng/data/ft_local/DressCode/lower_body/images", lower_garment),
                        lower_annotation_pair[lower_garment],
                        os.path.join("/apdcephfs_cq8/share_1367250/yijicheng/data/ft_local/DressCode/dresses/images", human)
                    )
                )
    ...
    for i in test_dataloader:
        ...
        for image_idx in range(4):
            ...
            masked_img_lower = pil_to_tensor(masked_img_lower.resize(human_img.size))
            masked_img = pil_to_tensor(masked_img.resize(human_img.size))

            # print(gt_human_sample.shape, gt_lower_sample.shape, image_out_lower.shape, gt_upper_sample.shape, image_out.shape)
            x_sample = torch.cat([gt_human_sample, pose_img_lower, masked_img_lower, gt_lower_sample, image_out_lower, pose_img, masked_img, gt_upper_sample, image_out], dim=2)
